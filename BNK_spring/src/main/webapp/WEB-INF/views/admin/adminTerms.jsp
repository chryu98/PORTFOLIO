...
input[type="text"], input[type="file"], select, button {
	...
	color: #111827;
	outline: none;
	transition: border-color .18s, box-shadow .18s, transform .05s, filter
		.12s;
}
...
.pager button.is-active {
	background: #2563eb; /* ✅ 메인 컬러와 통일 */
	border-color: #2563eb;
	color: #fff;
} /* 총 개수/페이지 표시 줄 */
.pager .meta {
	flex: 0 0 100%; /* ✅ 새 줄로 내려가 중앙정렬 */
	text-align: center;
	font-size: 12px;
	color: #6b7280;
	margin-top: 4px;
...
<body>
	...
	<!-- 검색/필터 바 -->
	<div
		style="width: min(1100px, 92vw); margin: 0 auto 10px; display: flex; gap: 8px;">
	...
	<!-- 업로드 폼 -->
	<form id="uploadForm">
		<label>파일명:</label><br> <input type="text" id="pdfName"
			name="pdfName" required /><br>
		<br> <label>사용 여부:</label><br> <select id="isActive"
			name="isActive" required>
			<option value="Y">사용</option>
			<option value="N">미사용</option>
		</select><br>
		<br> <label>약관 범위:</label><br> <input type="radio"
			id="scopeCommon" name="termScope" value="common" required> <label
			for="scopeCommon">공통약관</label><br> <input type="radio"
			id="scopeSpecific" name="termScope" value="specific"> <label
			for="scopeSpecific">개별약관</label><br> <input type="radio"
			id="scopeSelect" name="termScope" value="select"> <label
			for="scopeSelect">선택약관</label><br>
		<br> <label>약관 PDF 파일 업로드:</label><br> <input type="file"
			id="file" name="file" accept="application/pdf" required /><br>
		<br>
	...
	<div id="editFormWrapper"
		style="display: none; border: 1px solid #ccc; padding: 15px; margin-top: 20px;">
		<h3>PDF 수정</h3>
		<form id="editForm">
			<input type="hidden" id="editPdfNo" /> <label>파일명:</label><br>
			<input type="text" id="editPdfName" required /><br>
			<br> <label>사용 여부:</label><br> <select id="editIsActive"
				required>
				<option value="Y">사용</option>
				<option value="N">미사용</option>
			</select><br>
			<br> <label>약관 범위:</label><br> <input type="radio"
				id="editScopeCommon" name="editTermScope" value="common" required>
			<label for="editScopeCommon">공통약관</label><br> <input
				type="radio" id="editScopeSpecific" name="editTermScope"
				value="specific"> <label for="editScopeSpecific">개별약관</label><br>
			<input type="radio" id="editScopeSelect" name="editTermScope"
				value="select"> <label for="editScopeSelect">선택약관</label><br>
			<br> <label>새 PDF 파일 (선택):</label><br> <input type="file"
				id="editFile" accept="application/pdf" /><br>
			<br>
	...
	<div id="pdfModal"
		...
		<div
			style="width: min(1000px, 92vw); height: 80vh; margin: 6vh auto; background: #fff; border-radius: 12px; overflow: hidden; position: relative; box-shadow: 0 20px 50px rgba(0, 0, 0, .25);">
			<button id="pdfClose"
				style="position: absolute; right: 12px; top: 10px; z-index: 2; border: 1px solid #d1d5db; background: #fff; border-radius: 10px; padding: 8px 12px; cursor: pointer;">
				닫기</button>
  ...
  let sortKey = 'uploadDate';  // 'uploadDate' | 'pdfName'
  ...
  // ---------- 행 HTML ----------
  // ✅ JSP EL 충돌 피하려고 템플릿 보간(${}) 대신 문자열 연결 사용
  function rowHtml(pdf) {
    ...
    return ''
      ...
      +   '<td>'+pdf.pdfNo+'</td>'
      +   '<td>'
      +     '<a href="/admin/pdf/view/'+pdf.pdfNo+'" class="js-open-preview" data-id="'+pdf.pdfNo+'">'
      +       escapeHtml(pdf.pdfName || '')
      +     '</a>'
      +   '</td>'
      ...
      +       '<input type="checkbox" class="js-active-toggle" data-id="'+pdf.pdfNo+'"'+(isY?' checked':'')+'>'
      +       '<span>'+(isY ? '사용중' : '미사용')+'</span>'
      +     '</label>'
      +   '</td>'
  ...
  async function loadPdfList() {
    try {
      ...
      applyFiltersAndRender(); // 필터+정렬+페이징
    } catch (err) {
  ...
  function applyFiltersAndRender() {
    ...
    const status = $('#statusFilter')?.value || ''; // '', 'Y', 'N'
    ...
    filtered.sort((a,b)=>{
      ...
      let r;
      if (sortKey === 'uploadDate') r = (new Date(va)) - (new Date(vb));
      else r = (''+va).localeCompare(''+vb, 'ko');
    ...
    ['common','specific','select'].forEach(scope=>{
      ...
      if (total === 0) {
        tb.innerHTML = '<tr><td colspan="9" style="padding:16px;color:#6b7280;">항목이 없습니다.</td></tr>';
        return;
      }
  ...
  function renderPager(scope, total, pageCount) {
    ...
    range.forEach(v=>{
      if (v === '...') parts.push('<span style="padding:0 4px;">…</span>');
      else parts.push(makeBtn(v, v===cur));
    });
  ...
  document.addEventListener('click', (e)=>{
    ...
    switch (action) {
      ...
      case 'next':  pageState[scope] = cur+1; break; // 상한은 렌더에서 보정
      case 'last':  pageState[scope] = Number.MAX_SAFE_INTEGER; break; // 렌더에서 보정
  ...
  // ---------- 캐시 헬퍼 ----------
  function getPdfFromCache(pdfNo) {
    return pdfCache.find(p => String(p.pdfNo) === String(pdfNo));
  }
  ...
  document.addEventListener('change', async (e)=>{
    ...
    try {
      ...
      fd.append('pdfNo', item.pdfNo);
      fd.append('pdfName', item.pdfName);
      fd.append('isActive', next);
      fd.append('termScope', item.termScope);
      const res = await fetch('/admin/pdf/edit', { method:'POST', body: fd, credentials:'include' });
      if (!res.ok) throw new Error(await res.text());
      toast('상태가 업데이트 됐습니다.');
    } catch (err) {
      console.error(err);
      item.isActive = prev; // 원복
      applyFiltersAndRender();
      alert('상태 변경 실패');
    }
  });
