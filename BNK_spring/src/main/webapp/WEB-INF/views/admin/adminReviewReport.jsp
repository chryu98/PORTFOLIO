...
button{height:36px;padding:0 14px;border-radius:10px;border:1px solid var(--line);background:#fff;cursor:pointer;transition:transform .15s,box-shadow .15s,background .15s;box-shadow:var(--shadow);}
...
.chip{display:inline-block;border:1px solid var(--line);border-radius:999px;padding:2px 8px;margin-right:6px;background:#fff}
</style>
      ...
      <div class="app-main" role="main">
        ...
        <div class="section">
          <div style="display:flex;align-items:center;gap:12px;">
            <h2 style="margin:0">카드별 통합 현황 (가입자×상품)</h2>
            <div class="muted">임시 유입/본신청 확정</div>
          </div>
        ...
        <div class="section">
          ...
          <div class="card" style="margin-top:8px">
            <div class="muted">신규 신청(임시) — 나이대 × 성별</div>
            <div id="tblDemoStarts" style="margin-top:6px"></div>
          </div>
          <div class="card" style="margin-top:12px">
            <div class="muted">발급 완료(본신청) — 나이대 × 성별</div>
  ...
  async function loadAll(){
    try{
      ...
      cacheProductsEnriched=cacheProducts.map(p=>({
        ...
        isCreditCard: p.isCreditCard // 백엔드가 내려주면 사용
      }));
      ...
      // 이미지 매핑 (combined / products 중 우선 있는 값 사용)
      imgByCardNo = {};
      ...
      // genderCode 보존해서 자세히 모달에서 정확히 매칭
      ...
      renderTable('tblProducts', [
        { key:'cardNo', label:'카드', align:'left',
          render:(r)=>{ 
            ...
            return `<div class="cell-card">${imgTag}<div class="card-meta"><div class="card-name">${nm}</div><div class="card-no muted">#${r.cardNo}</div></div></div>`;
        }},
        {key:'isCreditCard', label:'유형', align:'left', render:(r)=>toKoCredit(r.isCreditCard)},
        {key:'starts', label:'신규 신청(임시)', align:'right'},
        {key:'issued', label:'발급 완료(본신청)', align:'right'},
...
function renderSummary(k){
  ...
  const tempOpen  = k?.tempOpen ?? 0;

  // 백엔드 값 우선, 단 0이나 null이면 보정: confirmed / inflow
  let convPct = (typeof k?.cohortConversionPct === 'number') ? k.cohortConversionPct : NaN;
  if (!isFinite(convPct) || convPct <= 0) {
    convPct = (inflow > 0) ? Math.round((confirmed * 1000) / inflow) / 10 : 0;
  }

  const convColor = convPct>=50?'var(--good)':convPct>=20?'var(--warn)':'var(--bad)';
  document.getElementById('kpis').innerHTML = `
    <div class="card kpi"><div class="label">신규 신청(임시)</div><div class="value">${fmt(inflow)}</div></div>
    <div class="card kpi"><div class="label">발급 완료(본신청)</div><div class="value">${fmt(confirmed)}</div></div>
    <div class="card kpi"><div class="label">진행 중(임시보관)</div><div class="value">${fmt(tempOpen)}</div></div>
    <div class="card kpi"><div class="label">확정 전환율</div><div class="value" style="color:${convColor}">${fmt(convPct)}%</div></div>`;
}
  ...
  function renderCardSummaryTable(rows){
    renderTable('tblCardSummary', [
      ...
      { key:'startsTemp',    label:'신규 신청(임시)',   align:'right' },
      { key:'confirmed',     label:'발급 완료(본신청)', align:'right' },
      { key:'conversionPct', label:'확정 전환율(%)',    align:'right' }
    ], rows);
  }

  // ===== 가입자 인구통계 표 (자세히 버튼 포함, 모달에 이미지 표시) =====
  function renderDemoStarts(rows){
    const headers = [
      {key:'ageBand', label:'나이대', align:'left'},
      {key:'gender',  label:'성별',   align:'left'},
      {key:'cnt',     label:'건수(임시)', align:'right'},
      {key:'more',    label:'', align:'center',
  ...
  function renderDemoIssued(rows){
    const headers = [
      {key:'ageBand', label:'나이대', align:'left'},
      {key:'gender',  label:'성별',   align:'left'},
      {key:'cnt',     label:'건수(확정)', align:'right'},
      {key:'more',    label:'', align:'center',
  ...
  function rowsSummary(){ if(!cacheSummary) return []; const inflow=cacheSummary.tempInflow??0; const confirmed=cacheSummary.finalConfirmed??0; const tempOpen=cacheSummary.tempOpen??0; const conv=(inflow>0?Math.round(confirmed*1000/inflow)/10:0); return [{지표:'신규 신청(임시)',값:inflow},{지표:'발급 완료(본신청)',값:confirmed},{지표:'진행 중(임시보관)',값:tempOpen},{지표:'확정 전환율(%)',값:conv}]; }
  ...
  function rowsProducts(){ 
    ...
    return src.map(r=>({
      카드번호:r.cardNo,카드명:(r.cardName||`#${r.cardNo}`),
      유형:toKoCredit(r.isCreditCard),
      '신규 신청(임시)':r.starts||0,'발급 완료(본신청)':r.issued||0,'전환율(%)':r.conversionPct||0,_이미지:r.cardImg||''
    })); 
  }
  function rowsCardSummary(){ 
	  return (cacheCardSummary||[]).map(r=>{
	    ...
	    return {
	      카드번호: r.cardNo,
	      카드명: r.cardName,
	      '신규 신청(임시)': r.startsTemp || 0,
	      '발급 완료(본신청)': r.confirmed || 0,
	      '확정 전환율(%)': r.conversionPct || 0,
	      _이미지: img
	    };
	  });
	}
	  ...
	  await Promise.all(imgs.map(async img=>{ 
	    ...
	    try{ 
	      const res = await fetch(src, {
	        // 같은 오리진(/admin/proxy-img)은 세션이 필요할 수 있음
	        credentials: 'include',
	        mode: 'same-origin',
	        headers: { 'X-Requested-With': 'XMLHttpRequest' } // (선택)
	      });
	      ...
	      img.outerHTML = '<div style="width:60px;height:36px;border:1px solid #e5e7eb;border-radius:6px;display:flex;align-items:center;justify-content:center;font-size:10px;color:#94a3b8">NO IMG</div>'; 
	    }
	  })); 
	}
  ...
  function buildAllPreviewHTML(){ 
    const styleCell='padding:8px 10px;', headCell=styleCell+'text-align:center;font-weight:700;background:#f5f7fb;';
    const td=(v,align)=>`<td style="${styleCell}text-align:${align||'left'};">${v}</td>`;
    const th=v=>`<th style="${headCell}">${v}</th>`;
    const previewCSS=`<style>.preview-export td .card-img{display:block;width:60px;height:36px;object-fit:contain;border:1px solid #e5e7eb;border-radius:6px}.preview-export td .noimg{width:60px;height:36px;border:1px solid #e5e7eb;border-radius:6px;display:flex;align-items:center;justify-content:center;font-size:10px;color:#94a3b8}</style>`;
    const buildTable=(title, head, rows, renderRow)=>{
      ...
      return `<div class="export-section export-keep"><h3>${title}</h3><div style="color:#667085;font-size:12px;margin:-4px 0 8px 0">기간: ${periodLabel()} · 생성: ${generatedAt()}</div><div class="table-wrap"><table class="table"><thead>${thead}</thead><tbody>${tbody}</tbody></table></div></div>`;
    };
    const cardRows=rowsCardSummary();
    const cardSummaryTable=(()=>{
      const head=['이미지','카드번호','카드명','신규 신청(임시)','발급 완료(본신청)','확정 전환율(%)'];
      const body=cardRows.map(r=>{ const img=r._이미지?`<img class="card-img" src="${toProxied(r._이미지)}" alt="${r.카드명}">`:`<div class="noimg">NO IMG</div>`;
        return `<tr>${td(img)}${td('#'+r.카드번호)}${td(r.카드명)}${td(fmt(r['신규 신청(임시)']),'right')}${td(fmt(r['발급 완료(본신청)']),'right')}${td(fmt(r['확정 전환율(%)']),'right')}</tr>`;
      }).join('');
      return `<div class="export-section export-keep"><h3>카드별 통합 현황</h3><div class="table-wrap"><table class="table"><thead><tr>${head.map(th).join('')}</tr></thead><tbody>${body}</tbody></table></div></div>`;
    })();
    const productsRows=rowsProducts();
    const productsTable=(()=>{
      const head=['이미지','카드번호','카드명','유형','신규 신청(임시)','발급 완료(본신청)','전환율(%)'];
      const body=productsRows.map(r=>{ const img=r._이미지?`<img class="card-img" src="${toProxied(r._이미지)}" alt="${r.카드명}">`:`<div class="noimg">NO IMG</div>`;
        return `<tr>${td(img)}${td('#'+r.카드번호)}${td(r.카드명)}${td(r.유형)}${td(fmt(r['신규 신청(임시)']),'right')}${td(fmt(r['발급 완료(본신청)']),'right')}${td(fmt(r['전환율(%)']),'right')}</tr>`;
      }).join('');
      return `<div class="export-section export-keep"><h3>상품판매 현황</h3><div class="table-wrap"><table class="table"><thead><tr>${head.map(th).join('')}</tr></thead><tbody>${body}</tbody></table></div></div>`;
    })();
    return previewCSS+`<div class="preview-export">`+
      buildTable('요약',['지표','값'],rowsSummary())+
      cardSummaryTable+
      buildTable('가입자 현황 — 신규 신청(임시)',['나이대','성별','건수'],rowsDemographyStarts())+
      buildTable('가입자 현황 — 발급 완료(본신청)',['나이대','성별','건수'],rowsDemographyIssued())+
      productsTable+
      `</div>`;
  }

  async function ensureJsPDFReady(maxWaitMs=5000){ const t0=performance.now(); while(performance.now()-t0<maxWaitMs){ if(window.jspdf&&window.jspdf.jsPDF) return true; await new Promise(r=>setTimeout(r,50)); } throw new Error('jsPDF 로드 지연'); }
  async function buildAllPDFBlob(){
    ...
    const style=`#pdfTemp .table-wrap{overflow:visible;border:0;box-shadow:none;}#pdfTemp .card{box-shadow:none;border:1px solid #e5e7eb;border-radius:0;}#pdfTemp .kpi.card{border:1px solid #e5e7eb}#pdfTemp .muted{color:#334155}#pdfTemp table{border-collapse:collapse !important;width:100%;}#pdfTemp thead th{position:static !important;background:#f5f7fb !important;}#pdfTemp th,#pdfTemp td{border:1px solid #e5e7eb !important;}#pdfTemp .export-section{margin:14px 0;}#pdfTemp h3{margin:16px 0 8px 0;}#pdfTemp .export-keep{page-break-inside:avoid;}#pdfTemp td img{display:block;width:60px;height:36px;object-fit:contain;border:1px solid #e5e7eb;border-radius:6px;}`;
    ...
    let y=0; while(y<canvas.height){ const sliceH=Math.min(pxPerPage, canvas.height-y); pageCanvas.width=canvas.width; pageCanvas.height=sliceH; ctx.clearRect(0,0,pageCanvas.width,pageCanvas.height); ctx.drawImage(canvas,0,y,canvas.width,sliceH,0,0,canvas.width,sliceH); const imgData=pageCanvas.toDataURL('image/jpeg',0.95); const imgH=sliceH*imgW/canvas.width; pdf.addImage(imgData,'JPEG',margin,margin,imgW,imgH); y+=sliceH; if(y<canvas.height) pdf.addPage(); }
  ...
  async function buildAllXlsxBlob(){
    ...
    const addSheet=(name, rows, headerOrder)=>{
      ...
      ws.getRow(1).eachCell(c=>{ c.font={bold:true}; c.alignment={vertical:'middle',horizontal:'center'}; c.fill={type:'pattern',pattern:'solid',fgColor:{argb:'FFF5F7FB'}}; c.border={top:{style:'thin',color:{argb:'FFE5E7EB'}},left:{style:'thin',color:{argb:'FFE5E7EB'}},bottom:{style:'thin',color:{argb:'FFE5E7EB'}},right:{style:'thin',color:{argb:'FFE5E7EB'}}}; });
      for(let r=2;r<=ws.rowCount;r++){ ws.getRow(r).eachCell((c,idx)=>{ const isNum=typeof c.value==='number'; c.alignment={vertical:'middle',horizontal:isNum?'right':(idx===1?'center':'left')}; c.border={top:{style:'thin',color:{argb:'FFE5E7EB'}},left:{style:'thin',color:{argb:'FFE5E7EB'}},bottom:{style:'thin',color:{argb:'FFE5E7EB'}},right:{style:'thin',color:{argb:'FFE5E7EB'}}}; }); if(r%2===0){ ws.getRow(r).eachCell(c=>{ c.fill={type:'pattern',pattern:'solid',fgColor:{argb:'FFFBFDFF'}}; }); } }
      ws.columns.forEach(col=>{ col.width=Math.max(col.width,12); });
      return ws;
    };
    addSheet('Summary', rowsSummary(), ['지표','값']);
    addSheet('Card_Summary', rowsCardSummary().map(({_이미지,...rest})=>rest), ['카드번호','카드명','신규 신청(임시)','발급 완료(본신청)','확정 전환율(%)']);
    addSheet('Demography_New', rowsDemographyStarts(), ['나이대','성별','건수']);
    addSheet('Demography_Issued', rowsDemographyIssued(), ['나이대','성별','건수']);
    addSheet('Products', rowsProducts().map(({_이미지,...rest})=>rest), ['카드번호','카드명','유형','신규 신청(임시)','발급 완료(본신청)','전환율(%)']);
    const meta=wb.addWorksheet('Meta'); meta.columns=[{header:'항목',key:'항목',width:10},{header:'값',key:'값',width:30}]; meta.addRow({항목:'기간',값:periodLabel()}); meta.addRow({항목:'생성',값:generatedAt()}); meta.getRow(1).eachCell(c=>{ c.font={bold:true}; c.alignment={horizontal:'center'}; c.fill={type:'pattern',pattern:'solid',fgColor:{argb:'FFF5F7FB'}}; });
  ...
  document.addEventListener('click', async (e)=>{
    ...
    if(e.target.id==='btnPreviewAllPdf'){ try{ const b=await buildAllPDFBlob(); openIframePreview('전체 PDF 미리보기', b);}catch(err){ console.error(err); alert('PDF 미리보기 실패: '+(err?.message||err)); } }
    if(e.target.id==='btnDownloadAllPdf'){ try{ const b=await buildAllPDFBlob(); downloadBlob(`report_${periodLabel().replace(/\s+/g,'')}.pdf`, b);}catch(err){ console.error(err); alert('PDF 다운로드 실패: '+(err?.message||err)); } }
    if(e.target.id==='btnPreviewAllXlsx'){ try{ const html=buildAllPreviewHTML(); openHTMLPreview('전체 엑셀 미리보기', html);}catch(err){ console.error(err); alert('엑셀 미리보기 실패: '+(err?.message||err)); } }
    if(e.target.id==='btnDownloadAllXlsx'){ try{ const b=await buildAllXlsxBlob(); downloadBlob(`report_${periodLabel().replace(/\s+/g,'')}.xlsx`, b);}catch(err){ console.error(err); alert('엑셀 다운로드 실패: '+(err?.message||err)); } }

    // “자세히” 버튼 - 카드 썸네일 포함
    const btn = e.target.closest && e.target.closest('button[data-seg]');
    if(btn){
      const [kind, ageBand, gender] = btn.dataset.seg.split('|'); // gender: M/F/U
      // 서버에서 받아온 카드×인구통계 매트릭스 사용
      const segMap = (function(arr){
        const map={}; for(const r of (arr||[])){ const k=`${r.ageBand}|${r.gender}`; (map[k] ||= {rows:[]}).rows.push(r); } return map;
      })(cacheCardDemo);
      ...
      const html = `
        <div class="inner">
          ...
          <div class="table-wrap"><table class="table">
            <thead><tr><th class="left">카드</th><th>신규 신청(임시)</th><th>발급 완료(본신청)</th></tr></thead>
            <tbody>
              ${arr.map(r=>`
                <tr>
                  <td class="left">
                    <div class="cell-card">
                      ...
                      <div class="card-meta"><div class="card-name">${r.cardName}</div><div class="card-no muted">#${r.cardNo}</div></div>
                    </div>
                  </td>
