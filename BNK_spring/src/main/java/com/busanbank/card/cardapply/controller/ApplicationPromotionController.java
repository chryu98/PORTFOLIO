...
public class ApplicationPromotionController {
  ...
  public ResponseEntity<?> promote(@PathVariable("appNo") long appNo) {
    ...
    // 0) 잠금 시도 (없으면 0)
    Integer appLocked = 0, personLocked = 0;
    try { appLocked    = appMapper.lockTemp(appNo);    } catch (Exception ignore) {}
    try { personLocked = personMapper.lockTemp(appNo); } catch (Exception ignore) {}

    // 1) 최소 보장: FINAL조차 없고 TEMP도 없으면 404
    if (appLocked == 0 && appMapper.existsFinal(appNo) == 0) {
